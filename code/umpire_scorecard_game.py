...
# getting basic game information given the same game_id as the main function
...
def get_home_plate_umpire(game_id):
    try:
        ...
        if response.status_code == 200:
            ...
            umpire = next(
                (official['official']['fullName'] for official in umpires if official['officialType'] == 'Home Plate'),
                None)
...
def plot_game(game_id: int):
    try:
        ...
        storing_game_data = storing_game_data.strftime('%Y-%m-%d').replace(':', '_')# Replace colons with underscores
        storing_year = storing_game_data.split("-")[0]
        ...
        file_path2 = (
            f'../masterfiles/{storing_year}/teams/{home_team}/descriptions/{storing_game_data}_vs_{away_team}.txt')
        file_paths.append(file_path2)
        file_path3 = (
            f'../masterfiles/{storing_year}/teams/{away_team}/descriptions/{storing_game_data}_at_{home_team}.txt')
        ...
        def calculate_colors():
            ...
            return strike_color, ball_color, total_color, strike_accuracy, ball_accuracy, total_accuracy
        ...
        # main functionality to draw pitches on the plot
        def plot_pitch(pitch, sz_top, sz_bottom, sz_left, sz_right, max_distance=1.65):
            ...
            if not pitch.empty:
                ...
                if pitch['description'] == 'ball' or pitch['description'] == 'called_strike':
                    if pitch['type'] == 'S':
                        if not (sz_left <= x <= sz_right and sz_bottom <= y <= sz_top):
                            ...
                            if distance_outside_zone <= max_distance:
                                ...
                                callid = f"{game_id}_{pitch_number}"
                                insert_data = "INSERT INTO descriptions(game_id, missed_call, pitcher, batter, velocity, inning, count, base_runners, caption, callid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                                description_data = (game_id, pitch_number, get_player_name_rl(pitch['pitcher']),
                                                    get_player_name_rl(pitch['batter']),
                                                    pitch['release_speed'], pitch['inning'],
                                                    f"{pitch['balls']}-{pitch['strikes']}",
                                                    get_base_runners(pitch), 'Ball was called a strike', callid)
                        ...
                        if (sz_left <= x <= sz_right and sz_bottom <= y <= sz_top):
                            ...
                            insert_data = "INSERT INTO descriptions(game_id, missed_call, pitcher, batter, velocity, inning, count, base_runners, caption, callid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                            description_data = (game_id, pitch_number, get_player_name_rl(pitch['pitcher']),
                                                get_player_name_rl(pitch['batter']), pitch['release_speed'],
                                                pitch['inning'], f"{pitch['balls']}-{pitch['strikes']}",
                                                get_base_runners(pitch), 'Strike was called a ball', callid)
                            cursor.execute(insert_data, description_data)
                            description.commit()
                            description.close()'''
        ...
        if strike_accuracy != 100:
            ax.annotate(f"{strike_accuracy}%", xy=(-0.73, 1.04), fontsize=14, weight='bold', color=strike_color)
        else:
            ax.annotate(f"{strike_accuracy}%", xy=(-0.8, 1.04), fontsize=13, weight='bold', color=strike_color)

        if total_accuracy != 100:
            ax.annotate(f"{total_accuracy}%", xy=(-0.23, 4.32), fontsize=14, weight='bold', color=total_color)

        else:
            ax.annotate(f"{total_accuracy}%", xy=(-0.3, 4.32), fontsize=13, weight='bold', color=total_color)

        if ball_accuracy != 100:
            ax.annotate(f"{ball_accuracy}%", xy=(0.23, 1.04), fontsize=14, weight='bold', color=ball_color)
        else:
            ax.annotate(f"{ball_accuracy}%", xy=(0.16, 1.04), fontsize=13, weight='bold', color=ball_color)
        ...
        plt.savefig(
            f'../masterfiles/{storing_year}/umpire_files/{umpire}/scorecards/{storing_game_data}_{home_team}_{away_team}.png',
            dpi=300)
        plt.savefig(f'../masterfiles/{storing_year}/teams/{home_team}/scorecards/{storing_game_data}_vs_{away_team}.png',
                    dpi=300)
        plt.savefig(f'../masterfiles/{storing_year}/teams/{away_team}/scorecards/{storing_game_data}_at_{home_team}.png',
                    dpi=300)
        ...
        umpire_data = (
        game_id, umpire, home_team, away_team, total_accuracy, strike_accuracy, ball_accuracy, strike_count, ball_count,
        missed_strike_count, missed_ball_count, storing_game_data, image)
